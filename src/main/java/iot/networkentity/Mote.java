...
public class Mote extends NetworkEntity {
    ...
    public void setPath(Path path) {
       // if(pathNotContainsDoubles(path)) {
            this.path = path;
      //  }
    }

    private boolean pathNotContainsDoubles(Path path)
    {
        List<Long> path2 = new ArrayList<>();
        path.getWayPoints().forEach(
            waypoint -> {
        ...
        {
            if (path2.subList(i+1,path2.size()-1).contains(path2.get(i)))
            {
                return false;
            }
        }
