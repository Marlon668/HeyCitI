...
public class SensorEnvironment {
    ...
    public double getDataBetweenPointsFromTime(GeoPosition begin, GeoPosition end,long startTime, long endTime,double velocity, double interpollationDistance) {
        double totalPollution = 0;
        long lastUpdatetTime = 0;
        SensorEnvironment environment = new SensorEnvironment();
        totalPollution += getDataFromSensors(environment,begin,lastUpdatetTime,startTime);
        lastUpdatetTime = startTime;
        double totalDistance = MapHelper.distance(begin, end);
        ...
        for (int i = 0; i < amount; i++) {
            ...
            currentTime = (long)(distance*1000/velocity)*1000 + currentTime;
            totalPollution += getDataFromSensors(environment,newPoint,lastUpdatetTime,currentTime);
            lastUpdatetTime = currentTime;
            lastGeoPosition = newPoint;

        }
        totalPollution += getDataFromSensors(environment,end,lastUpdatetTime,endTime);
        return totalPollution/(amount+2);
    }

    public double getDataBetweenPointsFromTime(GeoPosition begin, GeoPosition end,long startTime,double interpollationDistance) {
        double totalPollution = 0;
     ...
     */
    public double getDataFromSensors(SensorEnvironment environment,GeoPosition position,long lastTime, long timeInMs) {
        ...
        while (lastTime <= timeInMs) {
            environment.getPoll().doStep(lastTime*1000000,environment.getSensors().get(0).getEnvironment());
            lastTime++;
        }
    ...
    public double getDataFromSensorsForTime(GeoPosition position,long timeinSec) {
        if(sensors.isEmpty()){
            return 0.0;
        }
        double total = 0;
        ...
        for(int i = 0; i < sensors.size(); i++){
            if (allDistances.get(i) == 0.0){
                return sensors.get(i).generateData(timeinSec)/getMaxOfSensors();
            }
            total += sensors.get(i).generateData(timeinSec)*allDistances.get(i);
        }
