...
public class RoutingApplication3 extends Application {
    // RoutingApplication  for KAStar with adaptationgoal is current path a given percentage worse than new path
    ...
    public RoutingApplication3(PathFinder pathFinder, Environment environment) {
    ...
    private void handleRouteRequest(LoraWanPacket message) {
        ...
        if(bufferKBestPaths.get(deviceEUI).size()==bufferSize) {
            ...
            bufferKBestPaths = new HashMap<>();
            if (this.routes.get(deviceEUI) != null) {
                //this.routes.remove(0);
                if (pathAnalyser.hasChangedEnough(this.routes.get(deviceEUI))) {
