...
public class GraphStructure {
    ...
    public void setDoubleConnection(){
        ArrayList<Connection> connectionsList = new ArrayList<>(connections.values());
        for (Connection connection : connectionsList) {
                if(!(connectionExists(connection.getTo(),connection.getFrom()))){
                    connections.put(newConnectionID,new Connection(connection.getTo(),connection.getFrom()));
                    newConnectionID += 1;
                }
            }
    }
    ...
    private void addConnection(long id, Connection connection) {
        ...
        if(!(connectionExists(connection.getFrom(),connection.getTo()))) {
            connections.put(id, connection);
            if (id >= this.newConnectionID) {
                this.newConnectionID = id + 1;
            }
            Connection reversedConnection = new Connection(connection.getTo(), connection.getFrom());
            addConnection(newConnectionID, reversedConnection);
        }
    }
